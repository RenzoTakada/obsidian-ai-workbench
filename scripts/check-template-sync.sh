...
check() {
  ...
  local extracted
  extracted="$(awk "/<< '${marker}'\$/{flag=1; next} /^${marker}\$/{flag=0} flag" scripts/install-claude.sh)"
  if [ -z "$extracted" ]; then
...
check FSPEC       templates/generic/_AI/Templates/feature-spec-template.md
check APLAN       templates/generic/_AI/Templates/action-plan-template.md
